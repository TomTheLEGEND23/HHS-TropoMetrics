...
```
Browser → Frontend (30080/30081) → Loads email-config.js
   ↓
Browser → Email API (30090/30091) → SMTP Server → Recipient
         (Exposed NodePort)          Gmail/etc
         (FastAPI Python)
```

- **Frontend**: Static HTML/JS served by nginx
- **Email API**: Exposed via NodePort for direct browser access
- **Backend**: Handles SMTP authentication securely
- **Secrets**: Stored in Kubernetes, injected as environment variables
- **Configuration**: `EMAIL_API_URL` environment variable sets API endpoint

**Port Assignments:**
| Environment | Frontend | Email API |
|------------|----------|-----------|
| Production | 30080 | 30090 |
| Development | 30081 | 30091 |
...
```javascript
// Automatically generated by docker-entrypoint.sh at container startup
// Production example:
window.EMAIL_CONFIG = {
    apiUrl: 'http://10.0.0.101:30090'  // External NodePort URL
};

// Development example:
window.EMAIL_CONFIG = {
    apiUrl: 'http://10.0.0.101:30091'  // External NodePort URL
};
```

The `EMAIL_API_URL` environment variable is set in the Kubernetes deployment manifest:
- Production: `http://10.0.0.101:30090`
- Development: `http://10.0.0.101:30091`
...
// Check API health (using configured URL)
fetch(`${EMAIL_CONFIG.apiUrl}/health`)
    .then(r => r.json())
    .then(data => console.log('API Status:', data))
    .catch(err => console.error('API Unreachable:', err));

// Direct API test (development)
fetch('http://10.0.0.101:30091/health')
    .then(r => r.json())
    .then(data => console.log('Dev API:', data));
```
